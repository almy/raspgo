...
type bus struct   {
	JourneyDirection   string  `json:JourneyDirection,String`
}
...
const (
	...
	sluri     = "http://api.sl.se/api2/realtimedepartures.json?key=d0a921619ab540dba14f9ee09c82c2ef&siteid=1299&timewindow=10"
	slotcount = 4
)
...
func getSLDataFromApi() (ret slresp)  {
	slresp, err := http.Get(sluri)
	if err != nil {
		log.Fatal(err)
	}
	defer slresp.Body.Close()
	body, err := ioutil.ReadAll(slresp.Body)
	if err != nil {
		log.Fatal(err)
	}
	return body
